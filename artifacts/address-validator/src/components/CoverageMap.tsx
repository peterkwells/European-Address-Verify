...
function DetailPanel({
  ...
  const { data: detail, isLoading } = useGetCoverage(countryCode, {
    query: { enabled: !!countryCode, queryKey: getGetCoverageQueryKey(countryCode) },
  });
...
export function CoverageMap() {
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.BASE_URL}europe-coverage.geojson`, { signal: controller.signal })
      .then((r) => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json() as Promise<GeoJsonData>; })
      .then((data) => { if (!controller.signal.aborted) { setGeoJson(data); setGeoLoading(false); } })
      .catch((err) => { if (!controller.signal.aborted) { console.error("Failed to load GeoJSON", err); setGeoError(true); setGeoLoading(false); } });
    return () => controller.abort();
  }, [coverageData, geoJson]);

  const coverageByCode: Record<string, CountryCoverage> = {};
  if (coverageData) {
    for (const c of coverageData.countries) coverageByCode[c.country_code] = c;
  }
  ...
  const styleFeature = useCallback(
    (feature?: GeoJsonFeature): PathOptions => {
      ...
      return {
        fillColor: coverage ? (TIER_FILL[coverage.tier] ?? "#94a3b8") : "#94a3b8",
        fillOpacity: 0.65,
        color: "#fff",
        weight: 1,
        opacity: 0.8,
  ...
  const onEachFeature = useCallback(
    (feature: GeoJsonFeature, layer: Layer) => {
      ...
      layer.on({
        ...
        click: () => { if (iso) setSelectedCode(iso); },
      });
    },
  ...
  return (
    <div>
      ...
      <div className="mb-3 flex flex-wrap gap-4 text-xs">
        {[1, 2, 3, 4].map((tier) => (
          <div key={tier} className="flex items-center gap-2">
            <span className="h-3 w-3 rounded-sm border border-white/30" style={{ backgroundColor: TIER_FILL[tier] }} />
            <span className="text-muted-foreground">{TIER_LABELS[tier]?.label}</span>
          </div>
        ))}
      ...
      >
        ...
        {!geoError && (
          <MapContainer center={[54, 15]} zoom={4} style={{ height: "100%", width: "100%" }} scrollWheelZoom zoomControl>
            <TileLayer
            ...
            {geoJson && (
              <GeoJSON
                ...
                style={(f) => styleFeature(f as GeoJsonFeature)}
                onEachFeature={(f, l) => onEachFeature(f as GeoJsonFeature, l)}
              />
            )}
          </MapContainer>
        )}
      ...
      <p className="mt-2 text-xs text-muted-foreground">
        Click any country for coverage details and to try validation. Map data &copy; OpenStreetMap, &copy; CARTO.{" "}
        <a href="https://github.com/datasets/geo-countries" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">
          Boundaries
        </a>{" "}
        (ODbL).
      </p>
